    ...
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between p-4 sm:p-5">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
          <button className="bg-white text-gray-900 text-sm font-medium px-4 py-1.5 rounded-full transition-colors">
            Course
          </button>
          {['Field Guides', 'Geology', 'Plans', 'Live Tour'].map((item) => (
            <button
              key={item}
              className="text-white/80 text-sm font-medium px-4 py-1.5 rounded-full hover:bg-white/20 hover:text-white transition-colors"
            >
              {item}
            </button>
          ))}
      ...
      >
        {/* Base image - z-10 */}
        <div
          className="absolute inset-0 bg-center bg-cover bg-no-repeat hero-anim hero-zoom z-10"
